...
if __name__ == '__main__':
    ...
    nodes.append(dst)
    for i in range(start_time, end_time, 1000*1000*1000):
        sat_only_graph = graphs[i].subgraph(nodes)
        top15_rtts.append(calculate_path_lengths(graphs[i], nx.shortest_simple_paths(graphs[i], src, dst, weight="weight")))

    top15_rtts = np.array(top15_rtts) * 1000.0 / 299792458.0
    ...
    for i in range(15):
        rtts = top15_rtts[:, i]
        plt.plot(x, rtts, path_colors[i % 13], linestyle='--')
