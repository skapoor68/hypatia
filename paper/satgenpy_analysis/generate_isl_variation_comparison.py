...
def load_data(src=1584):
    ...
    for i in range(1584, 1684):
        for j in range(i + 1, 1684):
            ...
            variation_rtts.append(variation_times)
            basic_rtts.append(basic_times)
            # ratio.append(variation_times / basic_times)
            # mapping[counter] = (i, j)
            # counter += 1
    return np.sort(np.array(variation_rtts).flatten()), np.sort(np.array(basic_rtts).flatten())

if __name__ == '__main__':

    variation, basic = load_data()
    ratio = basic/variation
    ...
    plt.xlabel("Ratio of path lengths for +Grid variations", fontsize=18)
    ...
    base_file = "paper_plots/rtt_ratios"
